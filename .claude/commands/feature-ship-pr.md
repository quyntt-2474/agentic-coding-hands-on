---
description: Review staged changes for danger/policy violations, commit the safe set, then open a PR to a required base branch
...
## Step 1 — Audit staged changes ONLY

Review ONLY what is already staged (added to the git index via `git add`). Ignore unstaged and untracked files entirely — they are not part of this review.

1. Gather the staged set:
   - `git diff --cached --name-status` (the staged files + their change type)
   - `git diff --stat --cached`
   - If there is NOTHING staged (`git diff --cached --quiet` exits 0) → STOP: tell the user to `git add` the files they want reviewed first.
2. Classify every staged file as **SAFE** or **DANGEROUS/INAPPROPRIATE**.
...
For excluded files:
- Unstage them: `git restore --staged <file>` (or `git reset HEAD <file>`).
...
## Step 3 — Commit the safe staged set

1. Do NOT stage anything new — only the files already in the index (minus any unstaged in Step 2) are committed. Never `git add -A`.
